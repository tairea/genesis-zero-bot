...
def parse_file(filepath):
    """Parse a markdown file and return (frontmatter_dict, body_str)."""
...
def update_title(frontmatter, body):
    ...
    if new_title != title:
        ...
        if num is not None:
            num_str = str(int(num)).zfill(3) if isinstance(num, (int, float)) else str(num)
...
def add_metaphor_intros(body):
    ...
    # Find the first content line (skip blank lines after frontmatter)
    lines = body.split('\n')
    first_content_idx = 0
...
def process_file(filepath):
    ...
    try:
        ...
        # Update title
        frontmatter, body, title_changes, new_filename = update_title(frontmatter, body)
